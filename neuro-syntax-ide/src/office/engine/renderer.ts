...
export function renderScene(
  ...
  for (const ch of characters) {
    ...
    // Only apply at assigned seats — zone tiles (sofas, bookshelves) have no chair.
    const sittingOffset = (ch.state === CharacterState.TYPE && !ch.workingAtZone) ? CHARACTER_SITTING_OFFSET_PX : 0;
...
function renderBubbles(
  ...
  for (const ch of characters) {
    ...
    const sittingOff = (ch.state === CharacterState.TYPE && !ch.workingAtZone) ? BUBBLE_SITTING_OFFSET_PX : 0;
