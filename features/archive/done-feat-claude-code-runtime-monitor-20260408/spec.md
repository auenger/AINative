...
### General Checklist
- [x] Rust `sysinfo` 进程检测仅扫描与 workspace 相关的进程（性能考虑）
- [x] 轮询间隔可配置（默认 2s）
- [x] 非Tauri环境（dev mode）使用 mock 数据保持 UI 可用
- [x] 进程匹配规则覆盖 `claude` / `claude-code` / `node.*claude` 等变体

## Merge Record
- **Completed**: 2026-04-08
- **Merged Branch**: feature/feat-claude-code-runtime-monitor
- **Merge Commit**: ab3b2dc
- **Archive Tag**: feat-claude-code-runtime-monitor-20260408
- **Conflicts**: none
- **Verification**: passed (6/6 Gherkin scenarios, TypeScript + Rust compilation clean)
- **Stats**: 7 files changed, 847 insertions(+), 28 deletions(-)
