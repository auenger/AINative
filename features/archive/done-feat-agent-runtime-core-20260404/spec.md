...
### General Checklist
- [x] `AgentRuntime` trait 定义完整（id、name、status、query、health_check）
- [x] ClaudeCodeRuntime 实现完成
- [x] CodexRuntime 实现完成
- [x] Runtime 注册表（Registry）实现
- [x] Tauri IPC Commands 暴露
- [x] 错误处理与降级逻辑

## Merge Record
- **Completed**: 2026-04-04T23:30:00Z
- **Merged Branch**: feature/feat-agent-runtime-core
- **Merge Commit**: 8c6c5c5
- **Archive Tag**: feat-agent-runtime-core-20260404
- **Conflicts**: types.ts (feat-settings-llm-config vs feat-agent-runtime-core) — resolved by keeping both type sections
- **Verification**: PASS (15/15 tasks, 5/5 Gherkin scenarios, cargo check pass, vite build pass)
- **Stats**: 1 commit, 8 files changed, 963 insertions, 2 deletions
